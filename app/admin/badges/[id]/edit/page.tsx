...
import adminBadgeService, { UpdateBadgeDto } from "@/services/adminBadgeService";
...
export default function EditBadgePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchBadge = async () => {
      try {
        const definitions = await adminBadgeService.getBadgeDefinitions();
        const badge = definitions.find(b => b.id === id);
        if (!badge) {
          setError("Badge definition not found.");
        ...
        setFormData({
          name: badge.name,
          iconKey: badge.iconKey || "Sparkles",
          scopeOfWork: badge.scopeOfWork,
          assessmentMethod: badge.assessmentMethod,
          isActive: badge.isActive,
        });
        
        // Load form schema directly (now that backend returns arbitrary JSON array)
        if (Array.isArray(badge.formSchema)) {
          setFormFields(badge.formSchema as FormField[]);
        }
      } catch (err: any) {
        setError(err.message || "Failed to load badge definition");
      } finally {
        setInitialLoading(false);
      }
    };
  ...
  const handleSave = async () => {
    try {
      ...
      await adminBadgeService.updateBadgeDefinition(id, payload);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-24">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Badge: {formData.name}</h1>
          <p className="text-[13px] text-gray-500 mt-1">
            Update the skill badge definition and modify its application form.
          </p>
        </div>
      </div>
